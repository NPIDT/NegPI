  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, probabilities, is_real_example):
        ...
        precision = tf.metrics.precision_at_thresholds(labels=label_ids, predictions=probabilities[:,1], thresholds=[0.0001,0.01,0.5], weights=is_real_example)
        print(precision)
        print('\n')
        recall = tf.metrics.recall_at_thresholds(labels=label_ids, predictions=probabilities[:,1], thresholds=[0.0001,0.01,0.5], weights=is_real_example)
        print(recall)
        print('\n')
...
def main(_):
  # Setting environment variable to see second GPU
  import os
